...
async def register(request):
    if not registation:
        return web.Response(text="Kayıt devre dışı")
    uid, password = utils.bot_common.new_account(app["redis"])
    return web.Response(text=f"HESAP BILGILERIN >>>>> ID - {uid}, "
                             f"SIFRE - {password}"
                             f"                             BU SUNUCU SWICHEN TARAFINDAN KODLANMIŞTIR. HİÇBİR TÜRLÜ İNKAR EDİLEMEZ. HAKLARI SWICHEN#6097 ÜZERİNE AİTTİR.")
